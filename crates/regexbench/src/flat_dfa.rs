// ABOUTME: `flat_dfa` -- a per-call constant-factor squeeze of the cached lazy DFA.
// ABOUTME: Same proven DFA core and meta-engine routing as `lazy_dfa` (literal-prefix
// ABOUTME: -> memchr prefilter, anchored -> Pike VM, else flat-table lazy DFA), but the
// ABOUTME: two routing verdicts are cached on the DFA (no per-call AST walks) and the
// ABOUTME: `cached_find` scratch buffers are reused across calls (no per-call allocation).
// ABOUTME: Byte-identical to the oracle (proven by the differential test below).

//! Leftmost-first/greedy matching via a lazy/hybrid DFA, with a literal fast-path.
//!
//! This is the `flat_dfa` matcher. It is the same meta-engine as `lazy_dfa` -- it keeps
//! the `memchr` prefilter for patterns with a leading literal prefix and runs a lazy DFA
//! only for non-literal pairs -- with a tighter per-call constant factor. The DFA core
//! (the cached flat-table transition/provenance machinery) is identical to `lazy_dfa`;
//! the only changes are around the call boundary:
//!
//! - **Cached routing verdicts.** `lazy_dfa` re-walks the AST twice on *every* `find_all`
//!   call (`leading_literal_prefix` + `has_anchors_ast`) to decide literal / anchored /
//!   DFA, even on a cache hit. On the tiny non-literal tail pairs (the dominant share of
//!   `main`'s *time*), that per-call AST work dominates: the DFA walk is only ~16% faster
//!   than the scalar Pike VM there. `flat_dfa` computes both verdicts once, at DFA build
//!   time, and stores them on the cached `Dfa`, so the cache-hit path is a length check
//!   and a bool read.
//! - **One cache probe, no clone.** The cross-call cache is looked up once with
//!   `get_mut` (one hash, no `String` clone on the hit path); build runs only on first
//!   sight of a pattern.
//! - **Reusable scratch.** `cached_find`'s two `Vec<usize>` buffers live on the `Dfa` and
//!   are cleared (not reallocated) each call, so a cached match allocates nothing.
//!
//! The scan stays byte-identical to the Pike VM: the differential property test below
//! (vs `thompson`, itself oracle-identical) and the corpus-pair test guard it.
...
thread_local! {
    ...
    /// (Separate from `lazy_dfa`'s cache: this DFA carries the cached routing verdicts
...
/// patterns are matched by the position-aware Pike VM instead of this matcher. Computed
/// once at build time and cached on the DFA, so the hot path never walks the AST.
...
struct Dfa {
    ...
    overflowed: bool,
    // --- per-call constant-factor fields (the `flat_dfa` additions) ---
    /// Cached leading-literal-prefix verdict (computed once at build). `None` or a
    /// sub-2-byte prefix means "no prefilter"; a >=2-byte prefix routes to `memchr`.
    literal_prefix: Option<Vec<u8>>,
    /// Cached anchor verdict (computed once at build). Anchored patterns route to the
    /// Pike VM because position-dependent assertions can't be cached.
    anchored: bool,
    /// Reusable `cached_find` scratch buffers: cleared (not reallocated) each call.
    scratch_curr: Vec<usize>,
...
impl Dfa {
    ...
    ///
    /// Uses the reusable `scratch_curr` / `scratch_next` buffers (taken out of `self` for
    /// the duration of the call so the `&mut self` transition/merge methods can borrow
    /// them, then restored on every exit). This makes a cached call allocation-free.
    fn cached_find(&mut self, input: &[u8], scan: usize) -> Option<(usize, usize)> {
        ...
        let mut curr_starts = std::mem::take(&mut self.scratch_curr);
        let mut next_starts = std::mem::take(&mut self.scratch_next);
        curr_starts.clear();
        curr_starts.resize(start_count, scan);
        next_starts.clear();
        ...
        loop {
            ...
            if self.overflowed {
                self.scratch_curr = curr_starts;
                self.scratch_next = next_starts;
                return None;
            }
...
/// the cached lazy DFA. The two routing verdicts are computed once (at DFA build) and
/// stored on the cached DFA, so the hot cache-hit path does one hash probe and no AST
/// walk.
pub fn find_all(re: &Regex, input: &[u8], out: &mut Vec<Span>) {
    DFA_CACHE.with(|cell| {
        let mut cache = cell.borrow_mut();
        // One hash on the hot cache-hit path (no `String` clone); build + the two routing
        // verdicts run only on the first sight of a pattern.
        let dfa = match cache.get_mut(re.source.as_str()) {
            Some(d) => d,
            None => {
                let prog = Program::new(&re.ast);
                let mut dfa = Dfa::build(prog.insts().to_vec());
                dfa.literal_prefix = crate::prefilter::leading_literal_prefix(&re.ast);
                dfa.anchored = has_anchors_ast(&re.ast);
                cache.insert(re.source.clone(), dfa);
                cache.get_mut(re.source.as_str()).expect("just inserted")
            }
        };
        if let Some(prefix) = &dfa.literal_prefix {
            if prefix.len() >= 2 {
...
mod tests {
    ...
    fn dfa_spans(re: &str, input: &str) -> Vec<(usize, usize)> {
        ...
        let mut out = Vec::new();
        Impl::FlatDfa.find_all(&compiled, input.as_bytes(), &mut out);
        out.iter().map(|s| (s.start, s.end)).collect()
    }

    #[test]
    fn flat_dfa_matches_thompson_on_basics() {
    ...
    #[test]
    fn flat_dfa_literal_route_matches_naive() {
    ...
    #[test]
    fn flat_dfa_routes_anchors_to_pike_vm() {
    ...
    #[test]
    fn flat_dfa_matches_thompson_on_corpus_pairs() {
        for path in ["corpora/main.bin", "corpora/pathological.bin"] {
            ...
            for pair in &pairs {
                ...
                Impl::FlatDfa.find_all(&re, &pair.input, &mut b);
                assert_eq!(
                    a, b,
    ...
    /// Differential property test: `FlatDfa` must equal `Thompson` (itself
    ...
    #[test]
    fn flat_dfa_equals_thompson_differentially() {
        let corners: &[(&str, &str)] = &[
            ("a*", ""),
            ("a*", "aaa"),
            ("a+", "aaaa"),
        ...
        let check = |pattern: &str, input: &str| {
            ...
            Impl::FlatDfa.find_all(&re, input.as_bytes(), &mut b);
